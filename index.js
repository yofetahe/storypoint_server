...
io.on("connection", (socket) => {
  ...
  socket.on("join_room", ({ roomId, userData }) => {
    if (io.sockets.adapter.rooms.has(roomId)) {
      ...
      rooms.forEach((room) => {
        if (room.id === roomId) {
          // validate userData
          room.attendees.push(validateUserData(userData, room));
          roomData = room;
        }
      });
      // Response to client
      io.to(roomId).emit("joined_attendee", {
        room: roomData,
        userId: userData.id,
      });
  ...
  socket.on("leave_room", ({ roomId, username, userId }) => {
    ...
    rooms.forEach((room) => {
      if (room.id === roomId) {
        let indx = room.attendees.findIndex(
    ...
    socket.emit("user_disconnected", roomId);
  });
  ...
  socket.on("close_room", (roomId) => {
    ...
    rooms.splice(indx, 1);
    io.to(roomId).emit("room_closed", "Room closed by facilitator!");
...
function validateUserData(userData, room) {
  const index = room.attendees.findIndex((att) => att.name === userData.name);
  if (index > -1) {
    userData.name = `${userData.name}_${getRandomCharacter()}`;
  }
  return userData;
}
